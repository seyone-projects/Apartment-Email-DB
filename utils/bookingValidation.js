...
async function validateAmenityBooking({
  ...
  if (amenity === AMENITY_TYPE_WHOLE_DAY) {
    const bookingExists = await isWholeDayBooking(amenityId, date);
    if (bookingExists) {
      return {
        status: false,
  ...
  if (amenity === AMENITY_TYPE_FREE) {
    const exists = await isFreeBookingExists(amenityId, bookingStartDate);
  ...
  if (amenity === AMENITY_TYPE_HALF_DAY) {
    const bookingExists = await isBookingExists(amenityId, date);
    if (bookingExists) {
      return { status: false, message: "Booking already exists for this day" };
    }
